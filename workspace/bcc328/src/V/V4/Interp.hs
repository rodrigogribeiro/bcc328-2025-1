...
initConf :: Code -> Conf
initConf funs
  = Conf 0 code main [] Map.empty
    where
      code = foldr (\ (Fun n ins) ac -> Map.insert n ins ac) Map.empty funs
      main = case Map.lookup (Var "main") code of
                Just is -> is
                _ -> error "Function main not found!"
...
call v
  = do
      oldConf <- get
      let instrs = maybe [] id (Map.lookup v (funs oldConf))
          newConf = Conf 0 (funs oldConf) instrs [] Map.empty
      put newConf
      exec
      val <- pop
      put oldConf
